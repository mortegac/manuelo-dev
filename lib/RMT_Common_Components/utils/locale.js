...
export const sanitizeNumber = (value, slugCode='en-us') => {
    ...
    const valueWithoutThousands= replaceAll(value.toString(), thousandSeparator, "");
    
    // 4- Replace separator decimal per "."
    const newValue = decimalSeparator === ',' ? 
...
export const addFormatCurrency = (value=0, slugCode='en-US')=>{
    ...
    try {
        ...
        const numberWithFormat = new Intl.NumberFormat(slugCode).format(String(value));
